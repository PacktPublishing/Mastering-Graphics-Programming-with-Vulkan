...
GpuTechnique* RenderResourcesLoader::load_gpu_technique( cstring json_path ) {
    ...
    shader_code_buffer.init( rkilo( 512 ), temp_allocator );
    ...
    if ( !shaders.is_null() ) {
        ...
        for ( sizet s = 0; s < shaders.size(); ++s ) {
            ...
            if ( name == "vertex" ) {
                ...
                pc.shaders.add_stage( code, strlen( code ), VK_SHADER_STAGE_MESH_BIT_NV );
            } else if ( name == "task" ) {
                pc.shaders.add_stage( code, strlen( code ), VK_SHADER_STAGE_TASK_BIT_NV );
            }
        }
    }
