...
void GpuDevice::init( const GpuDeviceCreation& creation ) {
    ...
    for ( u32 i = 0; i < thread_frame_pools.size; ++i ) {
        ...
        VkQueryPoolCreateInfo timestamp_pool_info{ VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO, nullptr, 0, VK_QUERY_TYPE_TIMESTAMP, creation.gpu_time_queries_per_frame * 2, 0 };
...
ShaderStateHandle GpuDevice::create_shader_state( const ShaderStateCreation& creation ) {
    ...
    for ( compiled_shaders = 0; compiled_shaders < creation.stages_count; ++compiled_shaders ) {
        ...
        if ( creation.spv_input ) {
            ...
            createInfo.pCode = reinterpret_cast< const u32* >( stage.code );
        } else {
            ...
            char* stage_define = string_buffer.append_use_f( "%s_%s", to_stage_defines( stage.type ), creation.name );
            ...
            char* final_spirv_filename = string_buffer.append_use( "shader_final.spv" );
            // TODO: add optional debug information in shaders (option -g).
            char* arguments = string_buffer.append_use_f( "glslangValidator.exe %s -V --target-env vulkan1.1 -o %s -S %s --D %s", temp_filename, final_spirv_filename, to_compiler_extension( stage.type ), stage_define );
#else
            char* glsl_compiler_path = string_buffer.append_use_f( "%sglslangValidator", vulkan_binaries_path );
            char* final_spirv_filename = string_buffer.append_use( "shader_final.spv" );
            char* arguments = string_buffer.append_use_f( "%s -V --target-env vulkan1.1 -o %s -S %s --D %s", temp_filename, final_spirv_filename, to_compiler_extension( stage.type ), stage_define );
#endif
            ...
            if ( optimize_shaders ) {
                ...
                char* spirv_optimizer_path = string_buffer.append_use_f( "%sspirv-opt.exe", vulkan_binaries_path );
                char* optimized_spirv_filename = string_buffer.append_use_f( "shader_opt.spv" );
                char* spirv_opt_arguments = string_buffer.append_use_f( "spirv-opt.exe -O --preserve-bindings %s -o %s", final_spirv_filename, optimized_spirv_filename );

                process_execute( ".", spirv_optimizer_path, spirv_opt_arguments, "" );

                // Read back SPV file.
                createInfo.pCode = reinterpret_cast< const u32* >( file_read_binary( optimized_spirv_filename, temporary_allocator, &createInfo.codeSize ) );

                file_delete( optimized_spirv_filename );
            }
            else {
                // Read back SPV file.
                createInfo.pCode = reinterpret_cast< const u32* >( file_read_binary( final_spirv_filename, temporary_allocator, &createInfo.codeSize ) );
            }
        ...
        spirv::parse_binary( createInfo.pCode, createInfo.codeSize, name_buffer, shader_state->parse_result );
